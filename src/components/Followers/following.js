...
const MapFolllowing=(props)=>{
    ...
    useEffect (()=>{
        const getFollowing = async()=>{
            await axios.get(`http://localhost:3001/getFollowedUsers/${userId}`)
    ...
    return(
        <div className="following-follower-parent-container">
            ...
            {followedUsers.map(user=>(
                ...
                <Avatar style={{display:"inline-block",verticalAlign:"middle",border: 0, objectFit: 'cover'}} 
                src={user.avatar===undefined?null:
                    `http://localhost:3001/image/${user.avatar.slice(58,user.avatar.length)}`} size={100}/>
                <Link to={{ pathname: '/user/dashboard', state: { userId: user._id.toString()} }}  style={{textDecoration:"none",color:"black"}} ><p>{user.userName}</p></Link>
