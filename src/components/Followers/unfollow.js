...
const Unfollow = (props) => {
  ...
  const handleFollowUser = async (event) => {
    ...
    await axios
      .get(
        `http://localhost:3001/follow/${userId}/${props.userIdToUnfollow.toString()}`
      )
  ...
  const handleUnfollowUser = async (event) => {
    ...
    await axios
      .get(
        `http://localhost:3001/unfollow/${userId}/${props.userIdToUnfollow.toString()}`
      )
      .then(function (response) {
        console.log(response);
        setUnfollow(true);
  ...
  if (!unfollow) {
    return (
      <button type="submit" onClick={(event) => handleUnfollowUser(event)}>
        Unfollow
      </button>
    );
  }
